...
public class PickupItem : Photon.MonoBehaviour, IPunObservable
{
    ...
    /// <summary>If this chatClient sent a pickup. To avoid sending multiple pickup requests before reply is there.</summary>
    ...
    public void Pickup()
    {
        if (this.SentPickup)
        {
            // skip sending more pickups until the original pickup-RPC got back to this chatClient
            return;
        }
    ...
    {
        // when this chatClient's RPC gets executed, this chatClient no longer waits for a sent pickup and can try again
        ...
        {
            // you could also skip callbacks for items that are not picked up by this chatClient by using: if (this.PickupIsMine)
