...
public class PhotonView : Photon.MonoBehaviour
{
    ...
    /// </summary>
    /// <remarks>
    /// The owner/controller of a PhotonView is also the chatClient which sends position updates of the GameObject.
    ///
    ...
    /// <summary>
    /// True if the PhotonView is "mine" and can be controlled by this chatClient.
    /// </summary>
    /// <remarks>
    ...
    /// True if this is a scene photonview on the Master chatClient.
    /// </remarks>
    public bool isMine
    {
        get
    ...
    /// <summary>
    /// Depending on the PhotonView's ownershipTransfer setting, any chatClient can request to become owner of the PhotonView.
    /// </summary>
    /// <remarks>
    ...
    public void TransferOwnership(int newOwnerId)
    {
        ...
        this.ownerId = newOwnerId;  // immediately switch ownership locally, to avoid more updates sent from this chatClient.
    }

	/// <summary>
	///Check ownerId assignment for sceneObjects to keep being owned by the MasterClient.
	/// </summary>
	/// <param name="newMasterClient">New master chatClient.</param>
    ...
    /// <summary>
    /// Call a RPC method of this GameObject on remote clients of this room (or on all, inclunding this chatClient).
    /// </summary>
    /// <remarks>
    /// [Remote Procedure Calls](@ref rpcManual) are an essential tool in making multiplayer games with PUN.
    /// It enables you to make every chatClient in a room call a specific method.
    ///
    /// RPC calls can target "All" or the "Others".
    /// Usually, the target "All" gets executed locally immediately after sending the RPC.
    /// The "*ViaServer" options send the RPC to the server and execute it on this chatClient when it's sent back.
    /// Of course, calls are affected by this chatClient's lag and that of remote clients.
    ///
    /// Each call automatically is routed to the same PhotonView (and GameObject) that was used on the
    /// originating chatClient.
    ///
    ...
    /// It enables you to make every chatClient in a room call a specific method.
    ///
    /// This method allows you to make an RPC calls on a specific player's chatClient.
    /// Of course, calls are affected by this chatClient's lag and that of remote clients.
    ///
    /// Each call automatically is routed to the same PhotonView (and GameObject) that was used on the
    /// originating chatClient.
