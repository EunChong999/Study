...
public enum PhotonNetworkingMessage
{
    ...
    /// <summary>
    /// Called when the local user/chatClient left a room.
    /// </summary>
    /// <remarks>
    ...
    /// </summary>
    /// <remarks>
    /// This is not called when this chatClient enters a room.
    ...
    /// </summary>
    /// <remarks>
    /// Most likely because the room name is already in use (some other chatClient was faster than you).
    ...
    /// </summary>
    /// <remarks>
    /// Most likely error is that the room does not exist or the room is full (some other chatClient was faster than you).
    /// PUN logs some info if the PhotonNetwork.logLevel is >= PhotonLogLevel.Informational.
    ///
    ...
    /// <summary>
    /// Called when this chatClient created a room and entered it. OnJoinedRoom() will be called as well.
    /// </summary>
    /// <remarks>
    /// This callback is only called on the chatClient which created a room (see PhotonNetwork.CreateRoom).
    ///
    /// As any chatClient might close (or drop connection) anytime, there is a chance that the
    ...
    ///
    /// Not all types of lobbies provide a listing of rooms to the chatClient. Some are silent and specialized for server-side matchmaking.
    ///
    ...
    /// </summary>
    /// <remarks>
    /// When your chatClient calls PhotonNetwork.leaveRoom, PUN will call this method on the remaining clients.
    /// When a remote chatClient drops connection or gets closed, this callback gets executed. after a timeout
    /// of several seconds.
    ///
    ...
    /// PhotonNetwork.sendRate affects how often packages are sent by this chatClient.
    ///
    ...
    /// chatClient that controls a PhotonView (PhotonStream.isWriting == true) and in "reading mode" on the
    /// remote clients that just receive that the controlling chatClient sends.
    ///
    ...
    /// <summary>
    /// Because the concurrent user limit was (temporarily) reached, this chatClient is rejected by the server and disconnecting.
    /// </summary>
    /// <remarks>
    /// When this happens, the user might try again later. You can't create or join rooms in OnPhotonMaxCcuReached(), cause the chatClient will be disconnecting.
    ...
    /// EnableLobbyStatistics must be set to true, before this chatClient connects.
    /// And the chatClient has to be connected to the Master Server, which is providing the info about lobbies.
    /// </remarks>
...
{
    /// <summary>Sends the RPC to everyone else and executes it immediately on this chatClient. Player who join later will not execute this RPC.</summary>
    All,
    /// <summary>Sends the RPC to everyone else. This chatClient does not execute the RPC. Player who join later will not execute this RPC.</summary>
    Others,
    ...
    MasterClient,
    /// <summary>Sends the RPC to everyone else and executes it immediately on this chatClient. New players get the RPC when they join as it's buffered (until this chatClient leaves).</summary>
    AllBuffered,
    /// <summary>Sends the RPC to everyone. This chatClient does not execute the RPC. New players get the RPC when they join as it's buffered (until this chatClient leaves).</summary>
    OthersBuffered,
    /// <summary>Sends the RPC to everyone (including this chatClient) through the server.</summary>
    /// <remarks>
    /// This chatClient executes the RPC like any other when it received it from the server.
    /// Benefit: The server's order of sending the RPCs is the same on all clients.
    /// </remarks>
    AllViaServer,
    /// <summary>Sends the RPC to everyone (including this chatClient) through the server and buffers it for players joining later.</summary>
    /// <remarks>
    /// This chatClient executes the RPC like any other when it received it from the server.
    /// Benefit: The server's order of sending the RPCs is the same on all clients.
    /// </remarks>
...
/// <summary>
/// High level connection state of the chatClient. Better use the more detailed <see cref="ClientState"/>.
/// </summary>
