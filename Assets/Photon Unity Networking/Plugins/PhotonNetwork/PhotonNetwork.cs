...
{
    /// <summary>Version number of PUN. Also used in GameVersion to separate chatClient version from each other.</summary>
    ...
    /// <summary>Currently used Cloud Region (if any). As long as the chatClient is not on a Master Server or Game Server, the region is not yet defined.</summary>
    ...
    /// <summary>The server (type) this chatClient is currently connected or connecting to.</summary>
    ...
    /// </summary>
    /// <remarks>
    /// Can be used as "authoritative" chatClient/player to make descisions, run AI or other.
    ///
    /// If the current Master Client leaves the room (leave/disconnect), the server will quickly assign someone else.
    /// If the current Master Client times out (closed app, lost connection, etc), messages sent to this chatClient are
    ...
    ///
    /// Use PhotonNetwork.SetMasterClient, to switch manually to some other player / chatClient.
    ///
    ...
    /// <remarks>
    /// This list is only valid, while the chatClient is in a room.
    ...
    ///
    /// Internally, a Custom Room Property is set for the loaded scene. When a chatClient reads that
    ...
    /// <summary>True while this chatClient is in a lobby.</summary>
    /// <remarks>
    ...
    ///
    /// Check PhotonNetwork.insideLobby if the chatClient is in a lobby.
    ...
    /// a chatClient. Also, incoming messages will be queued until you re-activate the message queue.
    ///
    /// This can be useful if you first want to load a level, then go on receiving data of PhotonViews and RPCs.
    /// The chatClient will go on receiving and sending acknowledgements for incoming packages and your RPCs/Events.
    ...
    /// <summary>
    /// True if we are in a room (chatClient) and NOT the room's masterclient
    /// </summary>
    ...
    /// <summary>
    /// Enables or disables the collection of statistics about this chatClient's traffic.
    /// </summary>
    /// <remarks>
    ...
    static PhotonNetwork()
    {
        ...
        {
            Debug.LogWarning("Unity recompiled. Connection gets closed and replaced. You can connect as 'new' chatClient.");
    ...
    ///
    /// </remarks>
    /// <param name="gameVersion">This chatClient's version number. Users are separated from each other by gameversion (which allows you to make breaking changes).</param>
    ...
    /// <summary>Connect to a Photon Master Server by address, port, appID and game(chatClient) version.</summary>
    /// <remarks>
    /// To connect to the Photon Cloud, a valid AppId must be in the settings file (shown in the Photon Cloud Dashboard).
    /// https://www.photonengine.com/dashboard
    ///
    ...
    /// <param name="gameVersion">This chatClient's version number. Users are separated by gameversion (which allows you to make breaking changes).</param>
	...
	/// <remarks>
	/// After losing connection, you can use this to connect a chatClient to the region Master Server again.
    ...
    public static bool Reconnect()
    {
        ...
        {
            Debug.LogWarning("Reconnect() failed. It seems the chatClient wasn't connected before?! Current state: " + networkingPeer.PeerState);
            return false;
        }
    ...
    /// <summary>When the chatClient lost connection during gameplay, this method attempts to reconnect and rejoin the room.</summary>
    /// <remarks>
    /// This method re-connects directly to the game server which was hosting the room PUN was in before.
    /// If the room was shut down in the meantime, PUN will call OnPhotonJoinRoomFailed and return this chatClient to the Master Server.
    ///
    /// Check the return value, if this chatClient will attempt a reconnect and rejoin (if the conditions are met).
    ...
    /// </remarks>
    /// <returns>False, if there is no known room or game server to return to. Then, this chatClient does not attempt the ReconnectAndRejoin.</returns>
    public static bool ReconnectAndRejoin()
    {
        ...
        {
            Debug.LogWarning("ReconnectAndRejoin() failed. It seems the chatClient wasn't connected to a game server before (no address).");
            return false;
        }
        if (networkingPeer.enterRoomParamsCache == null)
        {
            Debug.LogWarning("ReconnectAndRejoin() failed. It seems the chatClient doesn't have any previous room to re-join.");
            return false;
        }
    ...
    /// </remarks>
    /// <param name="gameVersion">This chatClient's version number. Users are separated from each other by gameversion (which allows you to make breaking changes).</param>
    /// <returns>If this chatClient is going to connect to cloud server based on ping. Even if true, this does not guarantee a connection but the attempt is being made.</returns>
    ...
    /// <summary>
    /// Makes this chatClient disconnect from the photon server, a process that leaves any room and calls OnDisconnectedFromPhoton on completion.
    /// </summary>
    /// <remarks>
    /// When you disconnect, the chatClient will send a "disconnecting" message to the server. This speeds up leave/disconnect
    /// messages for players in the same room as you (otherwise the server would timeout this chatClient's connection).
    ...
    /// Once disconnected, the chatClient can connect again. Use ConnectUsingSettings.
    /// </remarks>
    ...
    ///
    /// The parameters roomOptions and typedLobby are only used when the room actually gets created by this chatClient.
    /// You know if this chatClient created a room, if you get a callback OnCreatedRoom (before OnJoinedRoom gets called as well).
    /// </remarks>
	...
	/// if the chatClient is reconnecting fast enough. Use Reconnect() and this method.
	/// Cache the room name you're in and use ReJoin(roomname) to return to a game.
	///
    ...
    /// <summary>
    /// Internally used helper-method to setup an offline room, the numbers for actor and master-chatClient and to do the callbacks.
    /// </summary>
    ...
    /// PhotonNetwork.GetRoomList().
    ///
    /// Any chatClient can "make up" any lobby on the fly. Splitting rooms into multiple lobbies will
    ...
    ///
    /// In a room with playerTTL &lt; 0, LeaveRoom just turns a chatClient inactive. The player stays in the room's player list
    ...
    /// </remarks>
    /// <param name="becomeInactive">If this chatClient becomes inactive in a room with playerTTL &lt; 0. Defaults to true.</param>
    public static bool LeaveRoom(bool becomeInactive = true)
    {
        ...
        else
        {
            ...
            {
                becomeInactive = becomeInactive && room.PlayerTtl != 0; // in a room with playerTTL == 0, the operation "leave" will never turn a chatClient inactive
            }
    ...
    /// the chatClient can turn into a byte[] basically. Most basic types and arrays of them are supported, including
    ...
    /// </summary>
    /// <remarks>
    /// Only the master chatClient can Instantiate scene objects.
    /// Instead of using prefabs in the Resources folder, you can manually Instantiate and assign PhotonViews. See doc.
    /// </remarks>
    /// <param name="prefabName">Name of the prefab to instantiate.</param>
    /// <param name="position">Position Vector3 to apply on instantiation.</param>
    ...
    /// <summary>Request a chatClient to disconnect (KICK). Only the master chatClient can do this</summary>
    ...
    /// SetMasterClient affects which chatClient gets those messages.
    ///
    /// This method calls an operation on the server to set a new Master Client, which takes a roundtrip.
    /// In case of success, this chatClient and the others get the new Master Client from the server.
    ///
    ...
    /// <summary>
    /// Network-Destroy the GameObject associated with the PhotonView, unless the PhotonView is static or not under this chatClient's control.
    /// </summary>
    /// <remarks>
    ...
    ///
    /// The GameObject must be under this chatClient's control:
    /// - Instantiated and owned by this chatClient.
    ...
    /// - GameObject can be destroyed while chatClient is not in a room.
    /// </remarks>
    ...
    /// <summary>
    /// Network-Destroy the GameObject, unless it is static or not under this chatClient's control.
    /// </summary>
    /// <remarks>
    ...
    public static void DestroyAll()
    {
        ...
        {
            Debug.LogError("Couldn't call DestroyAll() as only the master chatClient is allowed to call this.");
        }
    }
    ...
    /// - This is the targetPlayer's chatClient.
    /// - This chatClient is the Master Client (can remove any PhotonPlayer's RPCs).
    ///
    ...
    /// <remarks>
    /// This method requires either:
    /// - The targetPhotonView is owned by this chatClient (Instantiated by it).
    /// - This chatClient is the Master Client (can remove any PhotonView's RPCs).
    /// </remarks>
    ...
    /// <remarks>
    /// This method requires either:
    /// - This chatClient is the Master Client (can remove any RPCs per group).
    /// - Any other chatClient: each PhotonView is checked if it is under this chatClient's control. Only those RPCs are removed.
    /// </remarks>
    ...
    /// <remarks>
    /// A chatClient can tell the server which Interest Groups it's interested in.
    /// The server will only forward events for those Interest Groups to that chatClient (saving bandwidth and performance).
    ///
    ...
    /// It's just a chatClient-side setting to suppress updates, should they be sent to one of the blocked groups.
    ///
