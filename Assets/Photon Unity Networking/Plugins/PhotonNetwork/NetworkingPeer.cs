...
public enum ClientState
{
    ...
    /// <summary>Final state of a room join/create sequence. This chatClient can now exchange events / call RPCs with other clients.</summary>
    Joined,
    ...
    {
        /// <summary>This chatClient creates a room, gets into it (no need to join) and can set room properties.</summary>
        CreateRoom,
...
{
    /// <summary>Server actively disconnected this chatClient.
    /// Possible cause: The server's user limit was hit and chatClient was forced to disconnect (on connect).</summary>
    ...
    /// <summary>Server actively disconnected this chatClient.
    /// Possible cause: Server's send buffer full (too much data for chatClient).</summary>
    ...
    /// <summary>The security settings for chatClient or server don't allow a connection (see remarks).</summary>
    /// <remarks>
    /// A common cause for this is that browser clients read a "crossdomain" file from the server.
    /// If that file is unavailable or not configured to let the chatClient connect, this exception is thrown.
    ...
    /// <summary>Timeout disconnect by chatClient (which decided an ACK was missing for too long).</summary>
...
internal class NetworkingPeer : LoadBalancingPeer, IPhotonPeerListener
{
    ...
    /// <summary>True if this chatClient uses a NameServer to get the Master Server address.</summary>
    ...
    /// <summary>The server this chatClient is currently connected or connecting to.</summary>
    /// <remarks>
    ...
    /// <summary>Internal flag to know if the chatClient currently fetches a friend list.</summary>
    ...
    /// <summary>The cloud region this chatClient connects to. Set by ConnectToRegionMaster().</summary>
...
#region Operations and Connection Methods
    ...
    /// </summary>
    /// <returns>False, if the conditions are not met. Then, this chatClient does not attempt the ReconnectAndRejoin.</returns>
    ...
    {
        // once encryption is availble, the chatClient should send one (secure) authenticate. it includes the AppId (which identifies your app on the Photon Cloud)
    ...
    /// <summary>
    /// Request the rooms and online status for a list of friends. All chatClient must set a unique username via PlayerName property. The result is available in this.Friends.
    /// </summary>
    /// <remarks>
    ...
    {
        enterRoomParamsCache = new EnterRoomParams();   // this is used when the chatClient arrives on the GS and joins the room
    ...
    /// </summary>
    /// <remarks>Calls callback method OnLeftLobby if this chatClient was in a lobby initially. Clears the lobby's game lists.</remarks>
    ...
    /// <summary>
    /// Called when "this chatClient" left a room to clean up.
    /// </summary>
    ...
    /// <summary>Picks the new master chatClient from player list, if the current Master is leaving (leavingPlayerId) or if no master was assigned so far.</summary>
        ...
        switch (operationResponse.OperationCode)
        {
            ...
            case OperationCode.AuthenticateOnce:
            {
                ...
                if (operationResponse.ReturnCode != 0)
                {
                    ...
                    {
                        Debug.LogError(string.Format("The appId this chatClient sent is unknown on the server (Cloud). Check settings. If using the Cloud, check account."));
                ...
                else
                {
                    ...
                    {
                        // optionally, OpAuth may return some data for the chatClient to use. if it's available, call OnCustomAuthenticationResponse
            ...
            case OperationCode.CreateGame:
            {
                ...
                else
                {
                    ...
                    {
                        if (PhotonNetwork.logLevel >= PhotonLogLevel.Informational)
                            Debug.LogWarning(string.Format("CreateRoom failed, chatClient stays on masterserver: {0}.", operationResponse.ToStringFull()));
                    ...
                    if (!string.IsNullOrEmpty(gameID))
                    {
                        ...
                        // sent with the chatClient's request before!
        ...
        switch (statusCode)
        {
            ...
            case StatusCode.EncryptionEstablished:
                ...
                if (this.Server == ServerConnection.NameServer)
                {
                    ...
                    {
                        // this chatClient is not setup to connect to a default region. find out which regions there are!
                ...
                // we might need to authenticate automatically now, so the chatClient can do anything at all
            ...
            //    Debug.Log(statusCode + ". This chatClient buffers many incoming messages. This is OK temporarily. With lots of these warnings, check if you send too much or execute messages too slow. " + (PhotonNetwork.isMessageQueueRunning? "":"Your isMessageQueueRunning is false. This can cause the issue temporarily.") );
            //    break;
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        switch (photonEvent.Code)
        {
            case PunEvent.OwnershipRequest:
            {
                ...
                if (PhotonNetwork.logLevel == PhotonLogLevel.Informational)
                    Debug.Log("Ev OwnershipRequest " + requestedView.ownershipTransfer + ". ActorNr: " + actorNr + " takes from: " + currentOwner + ". local RequestedView.ownerId: " + requestedView.ownerId + " isOwnerActive: " + requestedView.isOwnerActive + ". MasterClient: " + this.mMasterClientId + ". This chatClient's player: " + PhotonNetwork.player.ToStringFull());
            ...
            case PunEvent.Destroy:
                ...
                // Debug.Log("Ev Destroy for viewId: " + instantiationId + " sent by owner: " + (instantiationId / PhotonNetwork.MAX_VIEW_IDS == actorNr) + " this chatClient is owner: " + (instantiationId / PhotonNetwork.MAX_VIEW_IDS == this.LocalPlayer.ID));
    ...
    public void DestroyPlayerObjects(int playerId, bool localOnly)
    {
        ...
        // in that case, the creator becomes the owner again. every chatClient can apply this. done below.
        foreach (PhotonView view in this.photonViewList.Values)
        {
    ...
    public void RunViewUpdate()
    {
        ...
        while (enumerator.MoveNext())
        {
            ...
            // a chatClient only sends updates for active, synchronized PhotonViews that are under it's control (isMine)
    ...
    protected internal void SetLevelInPropsIfSynced(object levelId,bool initiatingCall, bool asyncLoading = false)
    {
        ...
        // current level is not yet in props, so this chatClient has to set it
        ...
        this.SendOutgoingCommands();    // send immediately! because: in most cases the chatClient will begin to load and not send for a while
    }
